// ═══════════════════════════════════════════════════════════
// WFM Data Layer
// DB 고도화 시: DEFAULT_* → API 호출로 교체, DataAPI 메서드를 fetch()로 대체
// ═══════════════════════════════════════════════════════════

const STORAGE_KEY = 'wfm_data_v1';
...
const DEFAULT_ASSIGNMENTS = [
  ...
  ..._mkA('m9','p3', 2025,1, 2025,6, 1.0,'상주'),
  ..._mkA('m9','p4', 2025,7, 2025,11, 1.0,'상주'),
  ..._mkA('m9','p2', 2025,10, 2025,12, 0.3,'비상주'),
  ..._mkA('m10','p1', 2025,1, 2025,8, 0.7,'상주'),
  ..._mkA('m10','p4', 2025,5, 2025,11, 0.5,'상주'),
  // ── 2026 ──
  ...
  ..._mkA('m9','p6', 2026,1, 2026,12, 1.0,'상주'),
  ..._mkA('m9','p9', 2026,6, 2026,8, 0.4,'비상주'),
  ..._mkA('m10','p5', 2026,1, 2026,6, 0.5,'상주'),
  ..._mkA('m10','p6', 2026,3, 2026,8, 0.5,'상주'),
  ..._mkA('m10','p8', 2026,7, 2026,12, 0.7,'상주'),
];

// ─── 컬러 팔레트 (멤버·프로젝트 색상 선택용) ───
const PRESET_COLORS = [
...
function loadData() {
  try {
    ...
    if (stored) {
      ...
      DATA.members     = p.members     || _deepCopy(DEFAULT_MEMBERS);
      DATA.projects    = p.projects    || _deepCopy(DEFAULT_PROJECTS);
      DATA.assignments = p.assignments || _deepCopy(DEFAULT_ASSIGNMENTS);
    } else {
      _resetToDefaults();
    }
  } catch(e) {
...
const DataAPI = {
  /* ── 멤버 ── */
  addMember(m)       { m.id = 'm' + Date.now(); DATA.members.push(m); saveData(); return m; },
  updateMember(id,u) { const i=DATA.members.findIndex(m=>m.id===id); if(i>=0){ DATA.members[i]={...DATA.members[i],...u}; saveData(); } },
  deleteMember(id)   { DATA.members=DATA.members.filter(m=>m.id!==id); DATA.assignments=DATA.assignments.filter(a=>a.memberId!==id); saveData(); },

  /* ── 프로젝트 ── */
  addProject(p)      { p.id = 'p' + Date.now(); DATA.projects.push(p); saveData(); return p; },
  updateProject(id,u){ const i=DATA.projects.findIndex(p=>p.id===id); if(i>=0){ DATA.projects[i]={...DATA.projects[i],...u}; saveData(); } },
  deleteProject(id)  { DATA.projects=DATA.projects.filter(p=>p.id!==id); DATA.assignments=DATA.assignments.filter(a=>a.projectId!==id); saveData(); },
  ...
  /* ── 초기화 ── */
  reset() { _resetToDefaults(); saveData(); },
};
