...
// 기준 역할 정의 (2개월 기준 M/M)
const WISENM_ROLES = [
  { id:'pm',    cat:'관리',   name:'PM',              base:2.0, scaleWithDur:true,  note:'전체 기간 상주' },
  { id:'pl',    cat:'관리',   name:'PL',              base:2.0, scaleWithDur:true,  note:'전체 기간 상주' },
  { id:'ta',    cat:'기술',   name:'TA',              base:1.0, scaleWithDur:false, note:'초기 아키텍처 집중' },
  { id:'infra', cat:'인프라', name:'인프라 담당',      base:0.5, scaleWithDur:false, note:'서버 4~5개 구성' },
  { id:'sec',   cat:'보안',   name:'인프라 보안',      base:0.5, scaleWithDur:false, note:'네트워크·서버 보안' },
  { id:'comp',  cat:'보안',   name:'보안성 심의',      base:0.3, scaleWithDur:false, note:'문서 및 심의 대응' },
  { id:'aos',   cat:'개발',   name:'Android 개발자',   base:2.0, scaleWithDur:true,  note:'법인폰 앱 개발' },
  { id:'be',    cat:'개발',   name:'백엔드 개발자',    base:2.0, scaleWithDur:true,  note:'서버 4~5개 / API' },
  { id:'fe',    cat:'개발',   name:'프론트엔드 개발자',base:1.0, scaleWithDur:true,  note:'관리콘솔 / 모니터링' },
];
...
function renderWisenmView() {
  ...
  WISENM_ROLES.forEach(r => {
    const mm = _wn.custom[r.id] !== undefined
      ? _wn.custom[r.id]
      : Math.round((r.scaleWithDur ? r.base * ratio : r.base) * 10) / 10;
    totalMM += mm;
    const catHeader = r.cat !== prevCat
      ? `<tr class="wn-cat-row"><td colspan="5" style="color:${catColors[r.cat]}">${r.cat}</td></tr>`
      : '';
    prevCat = r.cat;
    rolesHtml += catHeader + `
      <tr class="wn-role-row" data-role-id="${r.id}">
        <td class="wn-role-name">${r.name}</td>
        <td class="wn-role-scale">${r.scaleWithDur ? '기간 비례' : '고정'}</td>
        <td class="wn-role-mm">
          <input class="wn-mm-input" type="number" step="0.1" min="0" value="${mm}" data-role="${r.id}">
        </td>
        <td class="wn-role-unit">M/M</td>
        <td class="wn-role-note">${r.note}</td>
      </tr>`;
  });

  let varsHtml = WISENM_VARS.map(v => {
    const val = _wn.vars[v.id] !== undefined ? _wn.vars[v.id] : '';
    return `<tr class="wn-var-row">
      <td class="wn-role-name">${v.name}</td>
      <td class="wn-role-scale" colspan="2">
        <input class="wn-var-input" type="text" placeholder="${v.placeholder}" value="${val}" data-var="${v.id}">
      </td>
      <td class="wn-role-unit">${v.unit}</td>
      <td class="wn-role-note">${v.note}</td>
    </tr>`;
  }).join('');
  ...
  const html = `
    <div class="wn-wrap">
      <div class="wn-header">
        <div class="wn-title-block">
          <div class="wn-badge">WiseN TM</div>
          <div>
            <div class="wn-title">공수 산정 기준표</div>
            <div class="wn-subtitle">하드웨어 포함 고객사 납품 기준 · 커스터마이징 없음</div>
          </div>
        </div>
        <div class="wn-dur-block">
          <label class="wn-dur-label">프로젝트 기간</label>
          <div class="wn-dur-controls">
            <button class="wn-dur-btn" id="wnDurMinus">−</button>
            <span class="wn-dur-val" id="wnDurVal">${dur}개월</span>
      ...
      <div class="wn-body">
        <div class="wn-section">
          <div class="wn-section-title">역할별 투입 공수</div>
          <table class="wn-table">
            <thead><tr>
              <th>역할</th><th>산정 방식</th><th style="width:90px">M/M</th><th>단위</th><th>비고</th>
            </tr></thead>
            <tbody>${rolesHtml}</tbody>
            <tfoot><tr class="wn-total-row">
              <td colspan="2">합계</td>
              <td class="wn-total-mm" id="wnTotalMM">${totalMM.toFixed(1)}</td>
              <td>M/M</td>
              <td></td>
            </tr></tfoot>
          </table>
        ...
        <div class="wn-section">
          <div class="wn-section-title">별도 산정 항목</div>
          <table class="wn-table">
            <thead><tr>
              <th>항목</th><th colspan="2">산정값</th><th>단위</th><th>비고</th>
            </tr></thead>
            <tbody>${varsHtml}</tbody>
          </table>
        </div>

        <div class="wn-assumptions">
          <div class="wn-assumptions-title">기준 가정</div>
          <ul>
            <li>서버 4~5개 단일 컨테이너 구성 (별도 인프라 없음)</li>
            <li>하드웨어(법인폰 포함) 고객사에 직접 납품</li>
            <li>CRM·콜센터 시스템 연동 없는 기본 구성 기준</li>
            <li>기간 비례 항목: PM/PL/AOS/BE/FE (기간 변경 시 자동 조정)</li>
            <li>고정 항목: TA/인프라/보안 (기간과 무관하게 고정)</li>
          </ul>
        </div>
      </div>
    </div>`;
  ...
  document.querySelectorAll('.wn-mm-input').forEach(inp => {
    inp.addEventListener('change', () => {
      ...
      if (!isNaN(v) && v >= 0) {
        ...
        const total = WISENM_ROLES.reduce((s, r) => {
          const mm = _wn.custom[r.id] !== undefined
            ? _wn.custom[r.id]
            : Math.round((r.scaleWithDur ? r.base * (_wn.dur / 2) : r.base) * 10) / 10;
          return s + mm;
        }, 0);
        document.getElementById('wnTotalMM').textContent = Math.round(total * 10) / 10;
      }
    });
  });

  // 별도 항목 편집
  document.querySelectorAll('.wn-var-input').forEach(inp => {
    inp.addEventListener('change', () => { _wn.vars[inp.dataset.var] = inp.value; });
  });
}
